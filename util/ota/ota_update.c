...
int main(int argc, char **argv)
{
	...
	if (ota_apply) {
		...
		if (rc) {
			log_err("apply failed rc %d error '%s'", rc,strerror(errno));
			ota_status(PB_ERR_FATAL);
			exit(1);
		}
		*/
		log_info("calling the system command");
		rc = system("/bin/sh /home/pi/ayla/bin/apply_ota.sh");
		log_info("system command executed with result '%d' error '%s'", rc,strerror(errno));
	}
	//ota_status(PB_DONE);
	/*if (ota_reboot) {
		log_info("rebooting");
