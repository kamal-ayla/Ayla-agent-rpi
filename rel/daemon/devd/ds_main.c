...
convert:
	...
	if (dir) {
		...
		if (*factory_file && file_is_dir(factory_file)) {
			factory_file[0] = '\0';
		}
		if (*startup_file && file_is_dir(startup_file)) {
			startup_file[0] = '\0';
		}
	}
	...
	if (*factory_file && *startup_file) {
		...
		len = snprintf(tmp, sizeof(tmp), "%s.bak", factory_file);
		if (len >= sizeof(tmp)) {
			log_err("%s file name was truncated", tmp);
			rc = -1;
		} else if (len < 0) {
		...
		len = snprintf(tmp, sizeof(tmp), "%s.bak", startup_file);
		if (len >= sizeof(tmp)) {
			log_err("%s file name was truncated", tmp);
		} else if (len < 0) {
			log_err("output error occured");
