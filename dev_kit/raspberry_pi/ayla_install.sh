...
install_webrtc_sdk() {
	...
	git apply $ayla_src_dir/dev_kit/raspberry_pi/kvs-webrtc-gst-pipeline-change-for-rpi.diff
	cd build
	cmake ..
	make -j
	cp -rf samples/kvsWebrtc* $ayla_install_dir/bin
}
...
install_kvsd_stream_webrtc() {
	...
	git apply $ayla_src_dir/dev_kit/raspberry_pi/kvs-webrtc-slave.diff
	cd build
	cmake ..
	if [ $? -ne 0 ]; then print_msg "Failed."; exit 1; fi
	make -j
...
install_kvsd_stream_master() {
    cd $working_dir
    cp -avf $ayla_src_dir/dev_kit/raspberry_pi/kvsd/kvsd_stream_master $working_dir
...
install_kvsd_stream_hls() {
    cd $working_dir
    cp -avf $ayla_src_dir/dev_kit/raspberry_pi/kvsd/kvsd_stream_hls $working_dir
...
install_camera_supporting_packages() {
	sudo apt-get update
	sudo apt-get install -y pkg-config cmake m4 git g++
	sudo apt-get install -y libssl-dev libcurl4-openssl-dev liblog4cplus-dev libgstreamer1.0-dev libgstreamer-plugins-base1.0-dev gstreamer1.0-plugins-base-apps gstreamer1.0-plugins-bad gstreamer1.0-plugins-good gstreamer1.0-plugins-ugly gstreamer1.0-tools libdbus-1-dev
}

install_ayla_modules() {
    echo "Building Ayla modules"
	...
	if [[ $no_release == 0 && -d rel ]]; then
		cd rel
		ayla_src_dir=$ayla_src_dir/rel
	fi
	print_msg "compiling Ayla source at $(pwd)"

	# Setup 'kvsd' app installation
	if [[ $support_kvsd != 0 ]]; then
	    cp -vf $ayla_src_dir/app/kvsd/stream_comm.h $ayla_src_dir/dev_kit/raspberry_pi/kvsd/kvsd_stream_master
	fi

	make -j INSTALL_ROOT=$ayla_install_dir clean
	echo "make -j PROD=$PROD_CODE APP=$app_to_build INSTALL_ROOT=$ayla_install_dir NO_WIFI=$no_wifi install"
	make -j PROD=$PROD_CODE APP=$app_to_build INSTALL_ROOT=$ayla_install_dir NO_WIFI=$no_wifi install
	...
	# Create link to app
	if [[ $support_kvsd != 0 ]]; then
	    if [[ "$app_to_build" != "$app_to_build_default" ]]; then
	        ln -sf $ayla_install_dir/bin/$app_to_build $ayla_install_dir/bin/$app_to_build_default
        fi
	fi
...
print_usage() {
	...
	echo "  -o, --no_release    Ignores release directory when building Ayla modules"
	...
	echo "  -f, --camera        Installs camera related software"
...
if [ $support_camera -eq 1 ]; then
	install_dir $ayla_install_dir/bin
	install_camera_supporting_packages
	install_wiringpi_from_source
	install_hls_sdk
fi
